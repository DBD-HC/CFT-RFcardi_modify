...
def get_all_files_in_directory_2(directory):
    ...
    for root, dirs, files in os.walk(directory):
        for i in range(len(files) // 6):
            ...
            sst_path = [file for file in files_path if re.search('sst', file)][0]
            ecg_path = [file for file in files_path if re.search('ecg', file)][0]
            anchor_path = [file for file in files_path if re.search('anchor', file)][0]
            ppi_path = [file for file in files_path if re.search('ppi', file)][0]

            sst_ecg_pair.append(os.path.join(
                root, sst_path))
            sst_ecg_pair.append(os.path.join(root, ecg_path))
            sst_ecg_pair.append(os.path.join(
                root, anchor_path))
            sst_ecg_pair.append(os.path.join(
                root, ppi_path))
...
class SpectrumECGDataset(Dataset):
    def __init__(self, sst_ecg_root, aug_snr = 100, align_length=200):
        ...
        self.index_select = np.random.choice(
                np.arange(len(self.all_sst_ecg_files)), size = int(40/100*len(self.all_sst_ecg_files)), replace=False) # used for abrupt nosing (20%)
    def __len__(self):
    ...
    def __getitem__(self, index):
        ...
        return sst_data, {'ECG_shape': ecg_data, 'Anchor': anchor_data}
