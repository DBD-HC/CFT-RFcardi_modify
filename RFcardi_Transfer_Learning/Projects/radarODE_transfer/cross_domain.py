...
from RFcardi_Transfer_Learning.Projects.radarODE_transfer.utils.utils import shapeMetric, shapeLoss, ppiMetric, ppiLoss, \
    anchorMetric, anchorLoss
...
def train(params, hyper, portion, radarODE_train_set=None, radarODE_test_set=None):
    kwargs, optim_param, scheduler_param = prepare_args(params)

    trainloader = torch.utils.data.DataLoader(
        dataset=radarODE_train_set, batch_size=params.train_bs, shuffle=True, num_workers=8, pin_memory=True,
        drop_last=True)
    testloader = torch.utils.data.DataLoader(
        dataset=radarODE_test_set, batch_size=params.test_bs, shuffle=True, num_workers=8, pin_memory=True,
        drop_last=True)
    ...
    def encoder_class():
        return backbone(in_channels=50, out_channels=backbone_out_channels)
...
def evaluate(params, hyper, portion, radarODE_test_set=None):
    kwargs, optim_param, scheduler_param = prepare_args(params)
    testloader = torch.utils.data.DataLoader(
        dataset=radarODE_test_set, batch_size=params.test_bs, shuffle=True, num_workers=8, pin_memory=True,
        drop_last=True)
...
if __name__ == "__main__":
    ...
    if params.is_pretrain:
        domain = 0
        params.save_name = f'SSL_'
        train_dataset, _, test_dataset = data_spliter.split_data(domain, train_idx=[i for i in range(4)],
                                                                 test_idx=[4], need_val=False)
        train(params, channel, portion=None, radarODE_train_set=train_dataset, radarODE_test_set=test_dataset)
    else:
        domain = 1
        params.load_path = f'/home/zhangyuanyuan/radarODE-Transfer/Model_saved/best_SSL_100_spar.pt'
        for test_user in user:
            if test_user < 0:
                continue
            params.save_name = f'Super_{test_user}_mse'
